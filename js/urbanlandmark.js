function loadUrbanLandmarkAttractions() {
    fetch('xml/attractions.xml')
        ...
        .then(str => {
            ...
            const urbanlandmarkList = document.getElementById("urbanlandmark-list");
            urbanlandmarkList.innerHTML = "";

            Array.from(attractions).forEach(attraction => {
                const category = attraction.getElementsByTagName("category")[0].textContent;

                if (category === "Urban Landmark") {
                    ...
                    urbanlandmarkList.appendChild(div);
                }
            });
        })
...
window.onload = loadUrbanLandmarkAttractions;
